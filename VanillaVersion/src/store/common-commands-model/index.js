...
const commonCommandsModel = {
  ...
  commandDecision: action((state, payload) => {
    state.commands = {
      ...state.commands,
      [payload.actionType]: { data: payload.data, error: payload.error, loading: payload.loading },
    };
  }),
  commandIsLoadingStart: action((state, payload) => {
    ...
    state.commands = {
      ...state.commands,
      [payload.actionType]: { ...state.commands[payload.actionType], error: false, loading: true },
    };
  }),
  commandIsLoadingSuccess: action((state, payload) => {
    ...
    state.commands = {
      ...state.commands,
      [payload.actionType]: { data: payload.data, error: null, loading: false },
    };
  }),
  commandIsLoadingFail: action((state, payload) => {
    ...
    state.commands = {
      ...state.commands,
      [payload.actionType]: { data: payload.data, error: payload.error, loading: false },
    };
  }),
  ...
  changeCamMode: thunk(async (actions, payload, { getState }) => {
    if (!payload.save) {
      ...
      const [camModeData, camModeDataError] = await handlePromise(
  ...
  getAvailableTools: thunk(async (actions, payload, { getState }) => {
    ...
    const [availableToolsData, availableToolsDataError] = await handlePromise(
    ...
    if (availableToolsData) {
      tools = [...tools, ...availableToolsData].map((tool, i) => {
        ...
        let id = i;
        if (!name.startsWith('object_locator')) id -= 1;
        return { id, name };
  ...
  setObjLocMatchThreshold: thunk(async (actions, payload) => {
    ...
    commandService
      .setObjLocMatchThreshold(payload)
      .then((r) => {
        actions.commandIsLoadingSuccess({
          actionType: commonCommandsTypes.OBJECT_MATCH_THRESHOLD,
          data: payload,
    ...
    commandService
      ...
      .then((response) => {
        ...
        return new Promise((resolve) =>
          resolve(commandService.getPixelCounterIntensityRange(payload.id))
        );
      })
      .then((response) => {
        actions.commandIsLoadingSuccess({
        ...
        return new Promise((resolve) =>
          resolve(commandService.getPixelCounterNoPixelsInRange(payload.id))
        );
      })
      .then((response) => {
        actions.commandIsLoadingSuccess({
  ...
  setPixelCounterForm: thunk(async (actions, payload, { getState }) => {
    ...
    commandService
      ...
      .then((response) => {
        ...
        return new Promise((resolve) =>
          resolve(
            commandService.setPixelCounterNoPixelsInRange({
              id: payload.id,
  ...
  getEdgePixelCounterForm: thunk(async (actions, payload, { getState }) => {
    ...
    commandService
      ...
      .then((response) => {
        ...
        return new Promise((resolve) =>
          resolve(commandService.getEdgePixelCounterStrength(payload.id))
        );
      })
      .then((response) => {
        actions.commandIsLoadingSuccess({
        ...
        return new Promise((resolve) =>
          resolve(commandService.getEdgePixelCounterNoPixelsInRange(payload.id))
        );
      })
      .then((response) => {
        actions.commandIsLoadingSuccess({
  ...
  setEdgePixelCounterForm: thunk(async (actions, payload, { getState }) => {
    ...
    commandService
      .setEdgePixelCounterStrength({ id: payload.id, value: payload.values[0] })
      .then((response) => {
        actions.commandIsLoadingSuccess({
          actionType: commonCommandsTypes.EDGE_PIXEL_COUNTER_STRENGTH,
        ...
        return new Promise((resolve) =>
          resolve(
            commandService.setEdgePixelCounterNoPixelsInRange({
              id: payload.id,
              min: payload.values[1],
              max: payload.values[2],
            })
          )
        );
      })
  ...
  setPatternScoreThreshold: thunk(async (actions, payload) => {
    ...
    commandService
      .setPatternScoreThreshold(payload)
      .then((response) => {
        actions.commandIsLoadingSuccess({
          actionType: commonCommandsTypes.PATTERN_SCORE_THRESHOLD,
          data: payload.value,
  ...
  getPatternScoreThreshold: thunk(async (actions, payload) => {
    ...
    commandService
      .getPatternScoreThreshold(payload.id)
      .then((response) => {
        actions.commandIsLoadingSuccess({
          actionType: commonCommandsTypes.PATTERN_SCORE_THRESHOLD,
          data: response[0],
