...
const referenceImagesModel = {
  ...
  getReferenceImages: thunk(async (actions, payload, { getState }) => {
    ...
    for (let id = 0; id < payload.values[1]; id += 1) {
      const request =
        id === payload.values[0]
          ? commandService.getActiveReferenceObject()
          : commandService.getReferenceObject(id);
  ...
  commandIsLoadingSuccess: action((state, payload) => {
    state.commands = {
      ...state.commands,
      [payload.actionType]: { data: payload.data, error: false, loading: false },
    };
  }),
  commandIsLoadingFail: action((state, payload) => {
    state.commands = {
      ...state.commands,
      [payload.actionType]: {
        ...state.commands[payload.actionType],
        error: payload.error,
        loading: false,
      },
    };
