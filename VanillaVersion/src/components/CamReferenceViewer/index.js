...
const CamReferenceViewer = () => {
  ...
  const changeActiveReferenceImage = useStoreActions(
    (actions) => actions.referenceImages.changeActiveReferenceImage
  );
  const changeCamMode = useStoreActions((actions) => actions.commonCommands.changeCamMode);
  const updateReferenceImages = useStoreActions(
    (actions) => actions.referenceImages.updateReferenceImages
  );
  ...
  return (
    <div className={classes.container}>
      ...
      >
        <Fade in={showModalWindow}>
          <Alert variant="filled" severity="warning" className={classes.blackText}>
            ...
            commonCommands[commonCommandsTypes.CAM_MODE].data.mode == 1 ? (
              <Typography align="center" color="primary" variant="h5">
                Камера в <b>EDIT MODE</b>. Что делаем?
              </Typography>
            ) : (
              <Typography align="center" color="primary" variant="h5">
                Камера будет переведена в <b>EDIT MODE</b>. Вы уверены?
              </Typography>
            )}
            <div className={classes.flexContainer}>
              ...
              <StyledIconButton color="secondary" onClick={() => setShowModalWindow(false)}>
                <CloseSharpIcon />
              </StyledIconButton>
            </div>
          </Alert>
        ...
        <Fade in={showSaveToFlashModal}>
          <Alert variant="filled" severity="warning" className={classes.blackText}>
            ...
            <div className={classes.flexContainer}>
              ...
              <StyledButton onClick={() => setShowSaveToFlashModal(false)}>
        ...
        {commands[referenceImagesTypes.ACTIVE_REFERENCE_OBJECT].data === null ? (
          <>
            {(commands[referenceImagesTypes.ACTIVE_REFERENCE_OBJECT].loading ||
              ...
              commands[referenceImagesTypes.COUNT_ACTIVE_REFERENCE_OBJECTS].loading) && <Spinner />}
          </>
        ) : (
          <>
            <div className={classes.camImage}>
              <StyledViewerImage
                ...
                src={
                  commands[referenceImagesTypes.ACTIVE_REFERENCE_OBJECT].data[
                    referenceNumber - 1
                  ] || ''
                }
              />
                ...
                <>
                  ...
                  {commonCommands[commonCommandsTypes.CAM_MODE].data ? (
                    <>
                      ...
                      {commonCommands[commonCommandsTypes.SAVE_TO_FLASH].loading && (
                        <Typography className={classes.warningColorText} variant="h4">
                          Идет сохранение...
                        </Typography>
                      )}
                    </>
      ...
      <div className={classes.settings}>
          ...
          <FormControl fullWidth className={classes.margin} variant="outlined" size="small">
            <Controller
              ...
              render={({ onChange }) => {
                ...
                for (
                  let i = 1;
                  i <= commands[referenceImagesTypes.COUNT_ACTIVE_REFERENCE_OBJECTS].data;
                  i++
                ) {
                  obj.push(
                    <MenuItem key={`MenuItem_${i}`} value={i}>
                      Рецепт №{i}
                    </MenuItem>
                  );
                }
        ...
        {showNestedSettings && (
          <Router>
            <CamReferenceSettings onUpdate={updateReferenceImages} uid={referenceNumber - 1} />
          </Router>
        )}
      </div>
    </div>
